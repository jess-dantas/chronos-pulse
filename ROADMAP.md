...
1. **R32 — REP-P (entregue em código, aguardando homologação)** — AFD (Anexo V) e AEJ (Anexo VI) seguem o leiaute oficial, com testes (CRC-16/CCITT-TRUE validado pelo vetor `"123456789"→2189` do próprio leiaute; SHA-256 encadeado; larguras fixas; sem linhas em branco; trailer com contagem). Já implementados: registros AEJ `02` (REPs utilizados, com nº do INPI via `numeroRegistroInpi`), `04` (horário contratual via `horarioContratual`/`codHorarioContratual`), `07` (ausências/banco de horas) e trailer `99`; **NSR reenumerado 1..N por estabelecimento (CNPJ)** em ordem cronológica. Para **homologação**: registrar o software no **INPI** (art. 91) e testar os campos de registro no AFD/AEJ; assinar AFD/AEJ com certificado **ICP-Brasil** (art. 86/88 via PFX); validar com o validador do MTE. Roteiro em `docs/inpi.md`. Backend: 317 testes verdes; app: 207.
